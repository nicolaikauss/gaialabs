...
export const LaserFlowWebGL: React.FC<LaserFlowWebGLProps> = ({
  className,
  style,
  wispDensity = 1,
  dpr,
  ...
  useEffect(() => {
    ...
    const { r, g, b } = hexToRGB(color || "#34d399")
...
export function LaserFlowWebGLFallback({ className }: { className?: string }) {
  return (
    <div
      className={className}
      style={{
        background: "radial-gradient(ellipse 60% 80% at 60% 60%, rgba(52,211,153,0.12) 0%, transparent 65%)",
      }}
    />
  )
}
